...
from src.utils import RandomComposeWithTarget, RandomCutPasteWithBoxes, RandomGaussianBlur, RandomBlackLinesWithTarget
from src.utils import RandomSubsetTransformWithTarget, WrapperForImageOnlyTransform
from src.utils import RandomResizedCropWithBoxes, RandomHorizontalFlipWithBoxes, RandomVerticalFlipWithBoxes
...
class YOLODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        boxes_xyxy[:, 0] = (boxes[:, 0] - boxes[:, 2] / 2) * original_w
        boxes_xyxy[:, 1] = (boxes[:, 1] - boxes[:, 3] / 2) * original_h
        boxes_xyxy[:, 2] = (boxes[:, 0] + boxes[:, 2] / 2) * original_w
        boxes_xyxy[:, 3] = (boxes[:, 1] + boxes[:, 3] / 2) * original_h

        target = {"boxes": boxes_xyxy, "labels": labels, "image_id": torch.tensor([idx])}
...
class DDETRTransform:
    def __init__(self, img_size, is_train, augmentations=None):
        self.img_size = img_size
        self.is_train = is_train
...
def train_one_epoch(model, criterion, data_loader, optimizer, device, epoch, print_freq=10):
    ...
    for i, (images, targets) in enumerate(data_loader): # Changed 'samples' to 'images'
        images = images.to(device)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        ##for debugging
        if i == 0:
            print("Batch shape:", images.tensors.shape)
            print(f"Number of targets: {len(targets)}")
            for ti, t in enumerate(targets[:2]):  # Show 2 examples max
                print(f"  Image {ti} - boxes: {t['boxes'].shape}, labels: {t['labels']}")
                print(f"    Boxes: {t['boxes'][:3]}")  # Print first 3 boxes only
            print("-" * 40)
...
def evaluate(model, criterion, data_loader, device):
    ...
    for samples, targets in data_loader:
        ...
        ##for debugging
        if i == 0:
            all_labels = torch.cat([t["labels"] for t in targets], dim=0)
            print("Unique class labels in batch:", all_labels.unique())
        ### end
...
if __name__ == "__main__":
    ...
    random_subset_with_target = RandomSubsetTransformWithTarget(image_only_transforms, num_choices=1)

    box_transforms = RandomComposeWithTarget([
        random_subset_with_target,
        RandomCutPasteWithBoxes(
            image_dir=args.train_img_dir,
            label_dir=args.train_ann_dir,
            p=0.8,
            max_iou=0.3
        ),
        RandomResizedCropWithBoxes(output_size=args.img_size),
        RandomHorizontalFlipWithBoxes(p=0.5),
        RandomVerticalFlipWithBoxes(p=0.5),
        RandomBlackLinesWithTarget(p=0.5),
    ], min_transforms=2, max_transforms=4)

    # --- Datasets and DataLoaders ---
    transform_train = DDETRTransform(
        img_size=416,
        is_train=True,
        augmentations=box_transforms  
        #augmentations=None
    )

    transform_val = DDETRTransform(
        img_size=416,
        is_train=False,
        augmentations=None  
    )
    ...
    ##for debugging
    print("\n🔍 Checking number of boxes in first 10 training samples:")
    for i in range(10):
        _, target = dataset_train[i]
        print(f"Image {i} - #boxes: {len(target['boxes'])}")
    ### end
